...
function removeFirstOccurrences(str, value) {
  const firstOccerrence = str.indexOf(value);
  return firstOccerrence === -1
    ? str
    : str.slice(0, firstOccerrence) + str.slice(firstOccerrence + value.length);
}
...
function removeLastOccurrences(str, value) {
  const lastOccerrence = str.lastIndexOf(value);
  return lastOccerrence === -1
    ? str
    : str.slice(0, lastOccerrence) + str.slice(lastOccerrence + value.length);
}
...
function isPalindrome(str) {
  const line = str.toLowerCase().replace(/[^a-z]/g, '');
  const middle = Math.floor(line.length / 2);
  const firstHalf = line.slice(0, middle);
  const secondHalf = line
    .slice(line.length - middle)
    .split('')
    .reverse()
    .join('');
...
function findLongestWord(sentence) {
  return sentence.split(' ').toSorted((a, b) => b.length - a.length)[0];
}
...
function reverseWords(str) {
  return str
    .split(' ')
    .map((everyWord) => everyWord.split('').reverse().join(''))
    .join(' ');
}
...
function encodeToRot13(str) {
  ...
  return str
    ...
    .map((letter) => {
      ...
      return indexLetter >= 0 ? alphabet13[indexLetter] : letter;
    })
    .join('');
}
